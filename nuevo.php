...
class RyuFlowManager {
    ...
    public function fetchRulesFromController() {
        $url = "http://{$this->controllerIp}:{$this->controllerPort}/stats/flow/{$this->dpid}";
        ...
        if ($result) {
            ...
            $rules = [];
            foreach ($data[$this->dpid] as $flow) {
                $rules[] = [
                    'prioridad' => $flow['priority'],
                    'puerto_entrada' => $flow['match']['in_port'] ?? '-',
                    'puerto_salida' => explode(':', $flow['actions'][0])[1] ?? '-',
                    'tipo_regla' => ($flow['match']['dl_type'] ?? '-') == 2048 ? 'IP' : 'ARP'
                ];
            }
            return $rules;
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['send_rule'])) {
        ...
        if ($priority && $inputPort && $outputPort && $ruleType) {
            $flowManager->sendFlowRule($priority, $inputPort, $outputPort, $ruleType);
        }
    } elseif (isset($_POST['fetch_rules'])) {
        $rules = $flowManager->fetchRulesFromController();
    }
} else {
        ...
        <div class="split-container">
            <div class="right-section">
                ...
                <form method="POST">
                    ...
                    <button type="submit" name="send_rule">ENVIAR REGLAS</button>
                </form>
                <form method="POST">
